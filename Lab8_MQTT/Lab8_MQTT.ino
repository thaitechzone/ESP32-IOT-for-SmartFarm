/* ============================================================================
 *  Lab8 : DHT11 -> MQTT (HiveMQ) + ควบคุมรีเลย์ 3 ตัวผ่าน MQTT + แสดงผล OLED
...
// ============================== ตัวแปรสถานะ ===============================
uint32_t dhtAt = 0, pubAt = 0, oledAt = 0, mqttRetryAt = 0;
...
void taskPublish(uint32_t now) {
  ...
  char bufT[12], bufH[12], bufJson[200];
  ...
  snprintf(bufJson, sizeof(bufJson),
           "{\"temp\":%.1f,\"humi\":%.1f,\"r1\":%d,\"r2\":%d,\"r3\":%d,"
           "\"rssi\":%d,\"uptime\":%u}",
           temp, humi,
           relays[0].on ? 1 : 0,
           relays[1].on ? 1 : 0,
...
void taskDisplay(uint32_t now) {
  ...
  oled.setTextSize(1);
  oled.setCursor(0, 0);
  oled.print("MQTT SmartFarm");
  oled.drawLine(0, 10, 127, 10, SSD1306_WHITE);

  // ---- ค่าเซนเซอร์ตัวใหญ่ ----
  oled.setTextSize(2);
  oled.setCursor(0, 15);
  if (sensorOK) oled.printf("%.1fC %.0f%%", temp, humi);
  else          oled.print("-- ERR --");
...
void setup() {
  ...
  Serial.println("\n=======================================================");
  Serial.println("  Lab8 : DHT11 -> MQTT + ควบคุมรีเลย์ผ่าน MQTT");
  Serial.println("=======================================================");
